...
/*
 * CCM processes AAD and the payload in a single pass each, so splitting
 * the AAD, adding AAD after the payload, or updating the payload again
 * must be rejected rather than silently producing a wrong tag.
 */
static int test_evp_aead_ccm_out_of_order_updates(int idx)
{
    const AEAD_DATA *info = &aead_list[idx];
    ...
    /* AAD split across two Update() calls. */
    if (!TEST_ptr(ctx_chunked_aad = EVP_CIPHER_CTX_new())
        || !TEST_true(EVP_EncryptInit_ex2(ctx_chunked_aad, info->ciph, key, iv,
                                          NULL))
        || !TEST_true(EVP_EncryptUpdate(ctx_chunked_aad, NULL, &outlen, NULL,
                                        ptlen))
        || !TEST_true(EVP_EncryptUpdate(ctx_chunked_aad, NULL, &outlen, aad1,
                                        aad1len))
        || !TEST_false(EVP_EncryptUpdate(ctx_chunked_aad, NULL, &outlen, aad2,
                                         aad2len))) {
        TEST_info("chunked AAD not rejected: idx=%d cipher=%s", idx, info->name);
        goto err;
    }

    /* AAD supplied after the payload. */
    if (!TEST_ptr(ctx_aad_after_payload = EVP_CIPHER_CTX_new())
        ...
        || !TEST_true(EVP_EncryptUpdate(ctx_aad_after_payload, ct, &outlen,
                                        pt, ptlen))
        || !TEST_false(EVP_EncryptUpdate(ctx_aad_after_payload, NULL, &outlen,
                                         aad1, aad1len))) {
        TEST_info("AAD after payload not rejected: idx=%d cipher=%s",
                  idx, info->name);
        goto err;
    }

    /* A second, zero-length payload Update() after the payload. */
    if (!TEST_ptr(ctx_payload_after_payload = EVP_CIPHER_CTX_new())
        ...
        || !TEST_true(EVP_EncryptUpdate(ctx_payload_after_payload, ct,
                                        &outlen, pt, ptlen))
        || !TEST_false(EVP_EncryptUpdate(ctx_payload_after_payload, ct,
                                         &outlen, pt, 0))) {
        TEST_info("payload after payload not rejected: idx=%d cipher=%s",
                  idx, info->name);
        goto err;
    }
...
int setup_tests(void)
{
    ...
    ADD_ALL_TESTS(test_evp_aead_ccm_out_of_order_updates, aead_list_n);
    return 1;
}
